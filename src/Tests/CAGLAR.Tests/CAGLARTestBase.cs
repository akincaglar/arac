...
namespace CAGLAR.Tests
{
    public abstract class CAGLARTestBase : AbpIntegratedTestBase<CAGLARTestModule>
    {
        private DbConnection _hostDb;
        private Dictionary<int, DbConnection> _tenantDbs; //only used for db per tenant architecture

        protected CAGLARTestBase()
        {
        ...
        protected void UsingDbContext(Action<CAGLARDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<CAGLARDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<CAGLARDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<CAGLARDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<CAGLARDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<CAGLARDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<CAGLARDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<CAGLARDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<CAGLARDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<CAGLARDbContext, Task<T>> func)
        {
            T result;
